...
import { PAGE_SIZE } from '../../hooks/useAgentsDirectory';
...
export default function AgentsPagination({
  page, totalPages, total, staleCount, onPageChange,
}: {
  page: number;
  totalPages: number;
  total: number;
  /** "875 nodos sin señal desde hace más de 6 horas" — nota fija del footer (handoff). */
  staleCount: number;
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-2.5 px-5 py-3.5">
      <div className="font-sans text-xs text-ink-400">
        {fmt(from)}–{fmt(to)} de {fmt(total)} nodos · {fmt(staleCount)} sin señal desde hace más de 6 horas
      </div>
