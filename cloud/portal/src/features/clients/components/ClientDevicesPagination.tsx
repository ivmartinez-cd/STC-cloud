...
import { DEVICE_PAGE_SIZE } from '../hooks/useClientDeviceDirectory';
...
/** Paginación de "Infraestructura de monitoreo" — idéntica a `ClientsPagination.tsx`
 * del listado de Clientes (README: "paginación idéntica a la de Clientes"), copiada
 * en vez de importada porque ese archivo está fuera de alcance ("ya terminado hoy,
 * no tocar") y su `PAGE_SIZE` es privado a esa feature. */
export default function ClientDevicesPagination({
  ...
  const from = page * DEVICE_PAGE_SIZE + 1;
  const to = Math.min((page + 1) * DEVICE_PAGE_SIZE, total);
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-2.5 px-5 py-3.5">
      <div className="font-sans text-xs text-ink-400">
        {fmt(from)}–{fmt(to)} de {fmt(total)} dispositivos
      </div>
