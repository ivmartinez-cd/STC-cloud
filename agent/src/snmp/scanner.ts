 ...
 */
import { captureDevice, type CaptureHint, type CaptureScope } from '../capture';
...
export async function readDevice(
  ...
  const out = await captureDevice({ ip, community, scopes: identityOnly ? ['identity'] : FULL, hint: h });
...
export async function readViaSNMP(ip: string, community: string, identityOnly = false): Promise<DeviceReading | null> {
  const out = await captureDevice({ ip, community, scopes: identityOnly ? ['identity'] : ['identity', 'meters', 'supplies'], hint: { driver: 'generic.printer-mib' } });
...
export async function readViaEWSCounters(ip: string, brand?: Brand, model?: string, hint?: CaptureHint, community = 'public'): Promise<DeviceReading | null> {
  const out = await captureDevice({ ip, community, scopes: ['meters'], hint: { ...hint, brand: brand ?? hint?.brand, model: model ?? hint?.model }, trustHint: true });
  return out?.reading ?? null;
}
...
export async function readViaEWSSupplies(ip: string, brand?: Brand, model?: string, hint?: CaptureHint, community = 'public'): Promise<DeviceReading | null> {
  const out = await captureDevice({ ip, community, scopes: ['supplies', 'alerts', 'trays'], hint: { ...hint, brand: brand ?? hint?.brand, model: model ?? hint?.model }, trustHint: true });
  return out?.reading ?? null;
}
