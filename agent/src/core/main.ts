    ...
    if (res.status === 401) {
      ...
      if (data.commands && data.commands.length > 0) {
        for (const cmd of data.commands) {
          // DeduplicaciÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â³n: Si ya lo procesamos (vÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â­a WSS o heartbeat anterior), lo saltamos
          if (cmd.id && processedCommandIds.has(cmd.id)) continue;
          
          if (cmd.id) {
            processedCommandIds.add(cmd.id);
            // Mantener el set limpio (ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Âºltimos 1000 IDs)
...
async function handleRemoteConfig(remote: any): Promise<void> {
  ...
  if (remote.ip_ranges && JSON.stringify(remote.ip_ranges) !== JSON.stringify(currentConfig.ipRanges)) {
    ...
    // Dispara scan inmediato si llegaron rangos donde antes no haba ninguno
...
async function checkForUpdate(serverUrl: string, force = false): Promise<boolean> {
  ...
  try {
    ...
    if ((UPDATE_PUBLIC_KEY_HEX as string) === 'PLACEHOLDER_RUN_GEN_KEYS_FIRST') {
      log('WARN', 'SEGURIDAD: firma Ed25519 no configurada ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â ejecutar installer/gen-keys.js y rebuild.');
    } else {
      try {
        ...
        if (!cryptoVerify(null, buffer, pubKey, sigBuf)) {
          log('ERROR', `VIOLACION DE INTEGRIDAD [Ed25519]: La firma del paquete de actualizacion NO es valida. URL: ${data.url} | Version: ${data.version} | Timestamp: ${new Date().toISOString()}. Actualizacion rechazada ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â posible ataque de cadena de suministro o paquete comprometido.`);
          isUpdating = false;
          return false;
        }
...
async function printStatus(): Promise<void> {
  ...
  // Health check rÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¡pido al servidor configurado (timeout 5s)
...
async function activate(): Promise<void> {
  ...
  try {
    ...
    // Propagamos exit code especÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â­fico si viene del bloque de respuesta HTTP
    if (e._stcExitCode) {
