...
**Versión:** 2.3 | **Fecha:** 2026-09-13 | **Clasificación:** Público / Auditoría IT
...
> **Cambios relevantes desde v2.2 (2026-09-11):** sin datos nuevos de dispositivo ni PII nueva, pero **cambia el alcance de una capacidad existente**: el acceso remoto a la EWS dejó de ser estrictamente de lectura (un GET por vez) y pasó a ser una sesión de navegación que **admite escritura sobre el equipo**, a la par de la función equivalente de HP SDS. Sigue siendo opt-in por monitor y desactivado de fábrica; se sumaron expiración de sesión por inactividad, custodia server-side de las cookies del equipo y una acción de auditoría propia para cada escritura (`REMOTE_EWS_WRITE`). Ver §4.1 y la tabla de riesgos del §5.
>
...
- **Acceso remoto a la EWS de un dispositivo** (`agents.remote_ews_enabled`): permite que un operador de STC, desde el portal, **navegue la interfaz web de una impresora del cliente como si estuviera en su red**, sin VPN. **Cambio de alcance respecto de versiones anteriores de este documento:** hasta ahora era un proxy de un único GET (estrictamente lectura); hoy es una sesión de navegación que **admite escritura sobre el equipo** — el operador puede autenticarse contra el firmware y cambiarle la configuración, con el mismo alcance que tendría alguien físicamente en la red del cliente con la contraseña del EWS. Esa contraseña la custodia el cliente: STC Cloud no la almacena ni la conoce.

  Controles: **desactivado por defecto** (requiere habilitación explícita por monitor, igual que HP SDS con su función equivalente); allowlist en dos capas (el cloud sólo permite IPs de dispositivos ya sincronizados de ese agente, el agente revalida contra su catálogo local antes de conectar); la sesión queda atada a **un solo equipo**, con la IP congelada del lado del servidor al abrirla; expira a los 30 minutos de inactividad; las cookies que emite la impresora se guardan **sólo del lado del servidor**, nunca en el navegador del operador; tope de 2 MB por respuesta; y auditoría en `audit_logs` de la apertura de sesión, de cada página visitada y —en acción propia y separada— de **toda escritura** (`REMOTE_EWS_WRITE`), siempre sin el contenido de los cuerpos.
...
| Acceso a red interna vía Remote EWS | **Bajo, mitigado** | Opt-in por monitor, allowlist en dos capas, sesión atada a un único equipo con IP congelada server-side, tope de tamaño de respuesta — ver §4.1 |
| Modificación de configuración de un equipo vía Remote EWS | **Medio, mitigado** | Riesgo nuevo: la función dejó de ser sólo lectura. Opt-in por monitor, sólo admin/operator, requiere la contraseña del EWS que custodia el cliente, sesión con expiración por inactividad, y **cada escritura auditada por separado** (`REMOTE_EWS_WRITE`) — ver §4.1 |
